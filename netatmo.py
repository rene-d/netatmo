...
def graph(args):
    ...
    W = 86400 * 7
    start = time.time()-W
    measures = ws.get_measure(date_begin=start, device_id=station["_id"], mtype="Temperature")
    #print(measures)
    width = 120
    values = [0] * width
    nvalues = [0] * width
    for t, v in measures["body"].items():
        x = int((int(t) - start) / (W / width))
        values[x] += v[0]
        nvalues[x] += 1
    for x in range(width):
        if nvalues[x] != 0:
            values[x] = round(values[x] / nvalues[x], 3)
...
def main():
    ...
    subparsers.add_parser("graph", help="temperature graph")
